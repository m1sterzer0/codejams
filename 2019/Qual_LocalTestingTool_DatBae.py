...
class Case:
  ...
  def Judge(self):
    ...
    print(f"Judge output: {self.__N} {len(self.__bad_set)} {self.__max_num_tries}",file=sys.stderr)
    ...
    for queries in range(self.__max_num_tries + 1):
      try:
        ...
        print(f"User output: {contestant_input}",file=sys.stderr)
      except Exception:
      ...
      if type(contestant_input) is str:
        ...
        if queries == self.__max_num_tries:
          ...
          xxx = self._answer_query(contestant_input)
          print(xxx)
          print(f"Judge output: {xxx}",file=sys.stderr)
          sys.stdout.flush()
      else:
        # Answer
