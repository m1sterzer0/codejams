...
function play(N::Int64,x::Vector{Int64})
    p = x[:]
    for i in 1:N
        w = []
        l = []
        for j in 1:2:2^N
            a,b = p[j],p[j+1]
            if a < b; push!(w,a); push!(l,b);
            else    ; push!(w,b); push!(l,a);
            end
        end
        p = vcat(w,l)
    end
...
function main(infn="")
    ...
    for qq in 1:tt
        ...
        possible::Vector{Bool}   = fill(false,2^N)
        guaranteed::Vector{Bool} = fill(true,2^N)
        players = collect(0:2^N-1)
        for x in permutations(players)
            xx = play(N,x)
            for p in xx[1:P];     possible[p+1] = true; end
            for p in xx[P+1:2^N]; guaranteed[p+1] = false; end
        end
        y,z = 0,0
        for i in 1:2^N
            if guaranteed[i]; y = i-1; end
            if possible[i]; z = i-1; end
        end
        print("$y $z\n")
    end
end
