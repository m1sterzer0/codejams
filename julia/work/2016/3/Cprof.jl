...
mutable struct Asteroid
    ...
    arcs::Array{Array{Tuple{Tuple{Float64,Float64},Int64},1},1}
end

Asteroid() = Asteroid(-1,0,Array{Tuple{Float64,Float64},1}(),Array{Array{Tuple{Tuple{Float64,Float64},Int64},1},1}())
...
function tryIt(dist::Float64,N::Int64,S::Int64,x0::Array{Float64,1},y0::Array{Float64,1},z0::Array{Float64,1},
    ...
    ##        dist of each other.
    interval::Array{Tuple{Float64,Float64},2} = fill((-1.0,-1.0),N,N)
    for i in 1:N-1
        for j in i+1:N
    ...
    for i in 1:N
        a::Array{Tuple{Tuple{Float64,Float64},Int64},1} = [(interval[i,j],j) for j in 1:N if interval[i,j][2] > 0]
        if length(a) == 0; continue; end
        sort!(a)  ## Prob worst line in here
        ii = 1
        while(ii <= length(a))
            jj = ii
            si,ei = max(0.0,a[ii][1][1]),a[ii][1][2]+S
            while (jj < length(a))
                if a[jj+1][1][1] > ei; break; end
                ei = max(a[jj+1][1][2]+S,ei)
                jj += 1
            end
            gr[i].numSubnodes += 1
    ...
    while !isempty(b)
        ...
        for arc in ast.arcs[ast.idx]
            if t > arc[1][2]; continue; end
            launchTime::Float64 = max(t,arc[1][1])
            push!(b,(launchTime,arc[2]))
        end
    end
    return false
end
