...
def gencase(small=True) :
    if small :
        numGophers = random.randrange(2,25+1)
        values = set()
        while len(values) < numGophers :
            values.add(random.randrange(1,1000000+1))
        gophers = list(values)
        gopherOrder = []
        while len(gopherOrder) < S :
            random.shuffle(gophers)
            gopherOrder += gophers
        return (numGophers,gopherOrder)
    else :
        numGophers = random.randrange(2,25+1)
        numValues = random.randrange(2,numGophers+1)
        values = set()
        ...
        while True :
            gophers = []
            for i in range(numGophers) :
                gophers.append(random.choice(values))
            if checkValidGophers(gophers) : break

        random.shuffle(gophers)
        gopherOrder = []
        tctype = random.choice([1,2,3])
        cutoff = 0.05 * random.random()
        while len(gopherOrder) < S :
            if tctype == 1 :
                random.shuffle(gophers)
            elif tctype == 3 :
...
def checkValidGophers(gophers) :
    ...
    for v in val :
        c = gophers.count(v)
        if mygcd == -1 : mygcd = c
        else : mygcd = math.gcd(mygcd,c)
        if mygcd == 1 : return True
    return False 
...
for i in range(NUM_TEST_CASES):
  #n,gopherOrder = gencase(True)
  n,gopherOrder = gencase(False)

  snacks = 0
  print(f"Starting testcase #{i+1}")
  if PRINT_INTERACTION_HISTORY:
  ...
  try:
    while True:
      ...
      if v < 0:  # The submitted value is a guess.
        ...
        JudgePrint(p, 1 if v >= gopherOrder[snacks] else 0)
        snacks += 1
  except Exception as e:
