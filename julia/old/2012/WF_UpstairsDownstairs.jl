...
function solve(N::Int64,K::Int64,A::Vector{Int64},B::Vector{Int64},C::Vector{Int64})
    ntot = sum(C[i] for i in 1:N)
    parr::Vector{Float64}    = fill(0.0,ntot)
    prea::Vector{Float64}    = fill(0.00,K)
    pres::Vector{Float64}    = fill(0.00,K)
    sufa2a::Vector{Float64}  = fill(0.00,K)
    sufa2s::Vector{Float64}  = fill(0.00,K)
    sufs2s::Vector{Float64}  = fill(0.00,K)
    idx = 1
    for i in 1:N
...
function main(infn="")
    ...
    for qq in 1:tt
        print("Case #$qq: ")
        N,K = gis()
        A::Vector{Int64} = fill(0,N)
        B::Vector{Int64} = fill(0,N)
