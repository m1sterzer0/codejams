...
function solveSmall(N::II, U::VI, V::VI, S::VI, C::VI)::PairI
    ...
    nodecosts::Vector{PairI} = [(nodecost[i],i) for i in 1:N]
    sort!(nodecosts)
    numski::II = 0; totcost::II = 0
    for (c,i) in nodecosts
        if i == 1; continue; end
        cc::II = 1_000_000; n::II = i
...
function heavyLightDecompose(N::II,r::II,adj::Vector{VI})
    ## We do this version without recursion to avoid stack overflow.
    q::Vector{TripI} = [(0,r,1)]
    order::VI = []
    parent::VI = fill(0,N)
    depth::VI  = fill(0,N)
    heavy::VI  = fill(0,N)
...
function solveLarge(N::II, U::VI, V::VI, S::VI, C::VI)
    adj::Vector{VI} = [VI() for i in 1:N]
    cost::VI = fill(0,N)
    cap::VI = fill(0,N)
    parent::VI = fill(0,N)
...
function pathQuery(st::SegTree,n::II,pos::VI,head::VI,parent::VI)
...
function pathUpdate(st::SegTree,n::II,inc::II,pos::VI,head::VI,parent::VI)
    while n != 0
        h = head[n]
...
function gencase(Nmin::II,Nmax::II,Smax::II,Cmax::II)
    N = rand(Nmin:Nmax)
    V = collect(2:N)
    shuffle!(V)
    S = rand(1:Smax,N-1)
...
function test(ntc::II,Nmin::II,Nmax::II,Smax::II,Cmax::II,check::Bool=true)
    pass = 0
    for ttt in 1:ntc
        (N::II,U::VI,V::VI,S::VI,C::VI) = gencase(Nmin,Nmax,Smax,Cmax)
